  ...
  function WindowedBlockDevice(config) {
    ...
    function readBlock(lba, buffer, callback) {
      if (lba < 0 || lba > getLastLogicalBlockAddress()) {
        throw new Error('readBlock에서 블럭 번호가 범위를 벗어났습니다. 허용 범위는 0에서 ' + getLastLogicalBlockAddress() + '까지이며, 요청 블럭은 ' + lba + '입니다.');
      }
    ...
    function writeBlock(lba, buffer, callback) {
      if (lba < 0 || lba > getLastLogicalBlockAddress()) {
        throw new Error('writeBlock에서 블럭 번호가 범위를 벗어났습니다. 허용 범위는 0에서 ' + getLastLogicalBlockAddress() + '까지이며, 요청 블럭은 ' + lba + '입니다.');
      }
      lba = lba + config.offset;
